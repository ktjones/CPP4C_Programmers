...
void MST::MST_Path(int s, const Graph & g, int c)
{
	...
	while (VisitedNodeCount < g.GraphAM.size())
	{
		...
		{
			if (g.GraphAM[CurrentNode][j].second > 0 && MST_Visited[j] != 1 && g.GraphAM[CurrentNode][CurrentNode].first == c)
			{
...
double MST::MST_Path_Size(int s, const Graph & g, int c)
{
