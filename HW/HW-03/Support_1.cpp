...
Graph::Graph(int n, double d) : Gr_Nodes(n), Gr_Density(d), Gr_Randomfill(false) 
{
	...
	for (int i=0;i<Gr_Nodes;i++)
	{
		...
		{
			GraphAM[i].push_back(make_pair(1,0));
		}
	}
...
void Graph::Graph_Neighbors(int x)
{
...
void Graph::Graph_AddEdge(int x, int y, double weight)
{
...
double Graph::Graph_GetNodeValue(int x)
{
	...
	//Our current Graph Structure does not allow for node values
